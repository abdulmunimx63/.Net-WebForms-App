...
{
    public partial class Take_Quantity : System.Web.UI.Page
    {
        int Id;
        string Name;
        int Quantity;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    if (quantityRowsAffected > 0)
                    {
                        ...
                        {
                            logCommand.Parameters.AddWithValue("@TypeOfTransaction", "Quantity Removed");
                            logCommand.Parameters.AddWithValue("@ItemName", Name);
                            logCommand.Parameters.AddWithValue("@ItemPrice", -1);
